...
const gamePlayReducer = (state, action) => {
  ...
  switch (action.type) {
    case types.START_GAME:
      const [computerShipLocations, separatedShipLocations] =
        generateComputerShipLocations();
      return {
        ...state,
    ...
    case types.UPDATE_SHIP_INDEX:
      ...
      return {
        ...
        lastHit: lastHit,
        hitList:
          action.data in state.computerSunkShips
            ? state.hitList
            : state.hitList.concat([action.data]),
      };
    ...
    case types.NEXT_COMPUTER_MOVE:
      if (!state.gameOver) {
        ...
        if (!state.shipIndex && !state.lastHit && state.hitList.length > 0)
          state.shipIndex = state.hitList[0];
        if (state.shipIndex) {
          ...
          if (!selectedGridIndex) {
            ...
            if (!selectedGridIndex) {
              ...
              selectedGridIndex = selectRandomSquare(
                state.gridIndices,
                state.coordinatesPicked,
    ...
    case types.ADD_TO_COMPUTER_SUNK_SHIPS:
      ...
      return {
        ...
        hitList: state.hitList.filter(
          (coordinate) => !(coordinate in sunkShipIds)
        ),
      };
